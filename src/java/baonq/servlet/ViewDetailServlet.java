...
public class ViewDetailServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            RentDetailDAO dao = new RentDetailDAO();
            List<RentDetails> list = dao.getRentByID(rentID);
            for(RentDetails dto:list){
                System.out.println(dto.toString());
            }
            request.setAttribute("listDetail", list);
        } catch (SQLException ex) {
            log("ViewDetailsServlet SQL:"+ex.getMessage());
        } catch (NamingException ex) {
            log("ViewDetailsServlet Naming:"+ex.getMessage());
        }finally{
